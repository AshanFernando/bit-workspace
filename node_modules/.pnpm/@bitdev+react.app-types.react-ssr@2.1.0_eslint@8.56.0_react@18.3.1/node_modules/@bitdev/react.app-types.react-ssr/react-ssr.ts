...
} from "./run-transformers.js";
import type { ReactSsrOptions } from "./react-ssr-type.js";
import { createRequire } from "node:module";

const require = createRequire(import.meta.url);

export class ReactSsr implements Application {
	constructor(
		/**
		 * name of the app.
		 */
		 ...
		 * root for the server.
		 */
		readonly serverRoot: string = "server.app-root.js",

		/**
		 * relative path to distribution dir.
		 * default is: dist
		 */
		readonly compileOutDir: string = "dist",

		/**
		 * root for the client.
		 */
		readonly clientRoot: string = "client.app-root.js",
		 ...
		 */
		readonly ssr: boolean = true,

		/**
		 * target exec file.
		 */
		readonly execFile: string = "server.cjs",
	) {}
	...
	async run(context: AppContext): Promise<ApplicationInstance> {
		...
		const entries = !isAbsolute(this.clientRoot)
			? [
					require.resolve(
						join(componentDir, this.compileOutDir, this.clientRoot),
					),
				]
			: [this.clientRoot];
		...
		const webpackConfig = this.transform(
			createConfig(options) as Configuration,
			{
				dev: true,
				appName: this.name,
				ssr: false,
		...
		const serverConfig = this.transform(
			createServerConfig({
				entries: [serverEntry],
				filename: "server.cjs",
		...
		if (this.ssr) {
			const clientCompiler = webpack(webpackConfig as any);
	...
	private getIndexHtml(rootPath: string) {
	...
	private async runBuild(compiler: Compiler): Promise<Stats | undefined> {
	...
	private transform(
		...
		const mutatedClientConfig = runTransformersWithContext(
			clientConfigMutator.clone(),
			this.transformers,
			context,
		);
	...
	async build(context: AppBuildContext): Promise<AppBuildResult> {
		...
		const ssrConfig = this.transform(
			createBuildConfig(
				{
					entries: [appEntry],
					name: "ssr",
	...
	static from(options: ReactSsrOptions) {
		return new ReactSsr(
			options.name,
			options.filename,
			options.defaultPort,
